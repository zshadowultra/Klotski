...
for (let i = 0; i < LEVELS.length; i++) {
  ...
  if (moves === -1) {
    ...
    // Try removing one piece
    for (let j = LEVELS[i].length - 1; j >= 0; j--) {
      if (LEVELS[i][j].type !== 'master') {
    ...
    // Try removing two pieces
    if (!fixed) {
      for (let j = LEVELS[i].length - 1; j >= 0; j--) {
        for (let k = j - 1; k >= 0; k--) {
          if (LEVELS[i][j].type !== 'master' && LEVELS[i][k].type !== 'master') {
            const testPieces = [...LEVELS[i]];
            testPieces.splice(j, 1);
            testPieces.splice(k, 1);
            const testMoves = solve(testPieces);
            if (testMoves !== -1) {
              console.log(`  Fixed by removing pieces ${k} and ${j}. New moves: ${testMoves}`);
              const lineIndex = lines.findIndex(l => l.includes(`[${LEVELS[i][0].x}, ${LEVELS[i][0].y}, ${LEVELS[i][0].w}, ${LEVELS[i][0].h}, '${LEVELS[i][0].type}']`));
              if (lineIndex !== -1) {
                const newCompact = testPieces.map(p => `[${p.x}, ${p.y}, ${p.w}, ${p.h}, '${p.type}']`).join(', ');
                lines[lineIndex] = `  [${newCompact}],`;
    ...
    if (!fixed) {
      console.log(`  Could not fix Level ${i + 1}.`);
    }
  }
}
